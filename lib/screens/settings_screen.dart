...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  Future<void> _confirmOldCode() async {
    ...
    if (mounted) {
      setState(() {
        _emailLoading = false;
        if (err != null) { _emailError = err; }
        else { _emailStep = _EmailStep.waitNewCode; _emailSuccess = 'Код отправлен на новый email'; _startCooldown(60); }
      });
    }
  }
    ...
    if (mounted) {
      setState(() {
        ...
        if (err != null) { _emailError = err; }
          ...
          _newEmailCtrl.clear(); _oldCodeCtrl.clear(); _newCodeCtrl.clear();
        }
      });
    }
  }
  ...
  void _resetEmailFlow() {
    setState(() {
      _emailStep = _EmailStep.idle; _emailError = null; _emailSuccess = null;
      _pendingNewEmail = ''; _newEmailCtrl.clear(); _oldCodeCtrl.clear(); _newCodeCtrl.clear();
    });
  }
  ...
  Future<void> _verifyKey() async {
    ...
    setState(() { _keyLoading = true; _keyError = null; _keySuccess = null; });
    if (!await SyncService().isOnline()) {
      setState(() { _keyLoading = false; _keyError = 'Нет интернета'; });
      return;
    }
    ...
    if (mounted) {
      setState(() {
        _keyLoading = false;
        _keyError = ok ? null : 'Неверный или просроченный ключ';
        _keySuccess = ok ? 'Ключ принят! AI чат доступен.' : null;
        ...
        leading: IconButton(
          icon: Container(
            ...
            decoration: BoxDecoration(
                color: cs.surfaceVariant, borderRadius: BorderRadius.circular(10),
        ...
        children: [
          ...
          if (user != null) ...[
            ...
            _SectionCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (_keyError != null) ...[const SizedBox(height: 10), _StatusMsg(text: _keyError!, isError: true)],
              if (_keySuccess != null) ...[const SizedBox(height: 10), _StatusMsg(text: _keySuccess!, isError: false)],
            ])),
            const SizedBox(height: 12),
            ...
            _SectionCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text('Текущий: ${user.username}',
                  style: GoogleFonts.notoSerif(color: cs.onSurfaceVariant, fontSize: 12.5, fontStyle: FontStyle.italic)),
              const SizedBox(height: 14),
              Row(children: [
                Expanded(child: _SettingsField(controller: _usernameCtrl, label: 'Новый никнейм', icon: Icons.person_outline_rounded)),
                const SizedBox(width: 10),
                SizedBox(height: 50, child: FilledButton(
              ...
              if (_usernameError != null) ...[const SizedBox(height: 10), _StatusMsg(text: _usernameError!, isError: true)],
              if (_usernameSuccess != null) ...[const SizedBox(height: 10), _StatusMsg(text: _usernameSuccess!, isError: false)],
            ])),
            const SizedBox(height: 12),
            ...
            _SectionCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 6),
              Text('После смены коды входа будут приходить на новый адрес',
                  style: GoogleFonts.notoSerif(color: cs.onSurfaceVariant, fontSize: 12.5)),
              const SizedBox(height: 14),
              _buildEmailFlow(cs),
              if (_emailError != null) ...[const SizedBox(height: 10), _StatusMsg(text: _emailError!, isError: true)],
              if (_emailSuccess != null) ...[const SizedBox(height: 10), _StatusMsg(text: _emailSuccess!, isError: false)],
            ])),
          ],
        ],
      ),
  ...
  Widget _buildEmailFlow(ColorScheme cs) {
    switch (_emailStep) {
      case _EmailStep.idle:
        return Row(children: [
          Expanded(child: _SettingsField(controller: _newEmailCtrl, label: 'Новый email', icon: Icons.email_outlined, keyboardType: TextInputType.emailAddress)),
          const SizedBox(width: 10),
          SizedBox(height: 50, child: FilledButton(
      ...
      case _EmailStep.waitOldCode:
        return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Введите код с текущего email', style: GoogleFonts.notoSerif(color: cs.onSurfaceVariant, fontSize: 13)),
          const SizedBox(height: 10),
          _OtpField(controller: _oldCodeCtrl),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: FilledButton(onPressed: _emailLoading ? null : _confirmOldCode,
                child: _emailLoading ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                    : Text('Подтвердить', style: GoogleFonts.notoSerif(fontWeight: FontWeight.w600)))),
            const SizedBox(width: 10),
            TextButton(onPressed: _resetEmailFlow, child: Text('Отмена', style: GoogleFonts.notoSerif(color: cs.onSurfaceVariant))),
          ]),
          if (_resendCooldown > 0)
            Text('Повторная отправка через $_resendCooldown с', style: GoogleFonts.notoSerif(color: cs.onSurfaceVariant, fontSize: 12))
          else
            TextButton(onPressed: _requestEmailChange, child: Text('Отправить снова', style: GoogleFonts.notoSerif(color: cs.primary, fontSize: 12))),
        ]);
      case _EmailStep.waitNewCode:
        return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Введите код с нового email ($_pendingNewEmail)', style: GoogleFonts.notoSerif(color: cs.onSurfaceVariant, fontSize: 13)),
          const SizedBox(height: 10),
          _OtpField(controller: _newCodeCtrl),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: FilledButton(onPressed: _emailLoading ? null : _confirmNewCode,
                child: _emailLoading ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                    : Text('Подтвердить', style: GoogleFonts.notoSerif(fontWeight: FontWeight.w600)))),
            const SizedBox(width: 10),
            TextButton(onPressed: _resetEmailFlow, child: Text('Отмена', style: GoogleFonts.notoSerif(color: cs.onSurfaceVariant))),
...
class _AppearanceCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return _SectionCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 14),
      Text('Тема', style: GoogleFonts.notoSerif(color: cs.onSurfaceVariant, fontSize: 12, letterSpacing: 0.5)),
      const SizedBox(height: 8),
      Row(children: [
        _ThemeChip(label: 'Тёмная', icon: Icons.dark_mode_rounded, selected: isDark, onTap: () => notifier.setMode(ThemeMode.dark)),
        const SizedBox(width: 10),
        _ThemeChip(label: 'Светлая', icon: Icons.light_mode_rounded, selected: !isDark, onTap: () => notifier.setMode(ThemeMode.light)),
      ]),
      const SizedBox(height: 18),
      Text('Цвет акцента', style: GoogleFonts.notoSerif(color: cs.onSurfaceVariant, fontSize: 12, letterSpacing: 0.5)),
      const SizedBox(height: 12),
      Wrap(
            ...
            child: Tooltip(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: selected ? [BoxShadow(color: opt.color.withOpacity(0.45), blurRadius: 8, spreadRadius: 1)] : null,
                ),
                child: selected ? Icon(Icons.check_rounded, color: Colors.white.withOpacity(0.9), size: 18) : null,
              ),
            ),
          );
        }),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: selected ? cs.primary.withOpacity(0.5) : cs.outline, width: selected ? 1.3 : 0.8),
        ),
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(icon, size: 16, color: selected ? cs.primary : cs.onSurfaceVariant),
          const SizedBox(width: 6),
          Text(label, style: GoogleFonts.notoSerif(color: selected ? cs.primary : cs.onSurfaceVariant, fontSize: 13, fontWeight: selected ? FontWeight.w600 : FontWeight.w400)),
        ]),
      ),
    );
  }
...
class _OtpField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    return TextField(
      controller: controller, keyboardType: TextInputType.number, maxLength: 6,
      inputFormatters: [FilteringTextInputFormatter.digitsOnly],
      textAlign: TextAlign.center,
      style: GoogleFonts.playfairDisplay(color: cs.onSurface, fontSize: 22, letterSpacing: 8),
      decoration: InputDecoration(
        counterText: '', hintText: '······',
        hintStyle: GoogleFonts.playfairDisplay(color: cs.onSurfaceVariant.withOpacity(0.4), fontSize: 22, letterSpacing: 8),
        enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: cs.outline, width: 0.9)),
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: cs.primary, width: 1.5)),
        filled: true, fillColor: cs.surface.withOpacity(0.5),
...
class _SectionTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    return Text(text, style: GoogleFonts.playfairDisplay(color: cs.onSurface, fontSize: 16, fontWeight: FontWeight.w600));
  }
}
...
class _SettingsField extends StatelessWidget {
  ...
  const _SettingsField({required this.controller, required this.label, required this.icon, this.keyboardType});
  @override
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    return TextField(
...
class _StatusMsg extends StatelessWidget {
  final String text; final bool isError;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Text(text, style: GoogleFonts.notoSerif(color: color, fontSize: 12.5))),
      ]),
    );
  }
}

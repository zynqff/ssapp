...
class _AiChatScreenState extends ConsumerState<AiChatScreen> {
  ...
  // Suggestion chips для пустого экрана
  ...
  Future<void> _send([String? preset]) async {
    ...
    final error = await ref.read(chatProvider(username).notifier).send(text);
    if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        automaticallyImplyLeading: false, // нет стрелки — это вкладка
        title: Row(children: [
          Container(
            width: 32, height: 32,
        ...
        actions: [
          GestureDetector(
            onTap: () async {
              ...
              if (ok == true) {
                ref.read(chatProvider(user.username).notifier).clear();
              }
            },
            child: Container(
              width: 38, height: 38,
      ...
      body: Column(children: [
        ...
        if (_offline)
          Container(
            ...
            child: Row(children: [
              Icon(Icons.wifi_off_rounded, size: 15, color: cs.error),
              const SizedBox(width: 8),
...
// ── Пустой экран с suggestion chips ──────────────────────────────────────────
...
class _EmptyState extends StatelessWidget {
  ...
  const _EmptyState({required this.onChipTap, required this.disabled});
          ...
          children: [
            ...
            Wrap(
              ...
              children: _suggestions.map((s) => ActionChip(
                ...
                onPressed: disabled ? null : () => onChipTap(s),
              )).toList(),
            ),
          ],
        ),
    ...
    return Align(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: isUser ? null : Border.all(color: cs.outline, width: 0.8),
        ),
        child: Text(
          msg.content,
          style: GoogleFonts.notoSerif(
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border(
            top: BorderSide(color: cs.outline.withOpacity(0.3), width: 0.8)),
      ),
      child: Row(children: [
        Expanded(
          child: Container(
